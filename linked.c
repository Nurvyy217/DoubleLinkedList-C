...
/**** Manajemen Memory ****/
addressKota Alokasi(const char *namaKota)
...
alamat Search (addressKota alamatKota, const char *namaWarga){
    alamat current = headWarga(alamatKota);
    while (current != Nil)
    {
        if (strcasecmp(nama(current), namaWarga) == 0)
        {
            return current;
        }
...
alamat SearchPrec (const char *namaWarga, addressKota alamatKota){
    alamat current = headWarga(alamatKota);
    if (current == Nil || nextNama(current) == Nil)
    {
        return Nil;
    }
    while (nextNama(current) != Nil)
    {
        if (strcasecmp(nama(nextNama(current)), namaWarga) == 0)
        {
            return current;
        }
        current = nextNama(current);
...
void InsertLast (List * L, address P){
    if (First(*L) == NULL) { // Jika List kosong
        First(*L) = P; // P langsung jadi elemen pertama
    } else {
        address last = First(*L);
        while (Next(last) != NULL) { // Cari elemen terakhir
            last = Next(last);
        }
        Next(last) = P;
    }
...
void DelFirst (List * L, address * P){
    if (First(*L) != NULL) { // Cek apakah List kosong
        *P = First(*L); 
        First(*L) = Next(First(*L)); 
        Next(*P) = NULL; // Putuskan hubungan dengan List
    }
}
...
void DelP (List * L, infotype X){
    ...
    if (P != Nil)
    {
        if (Prec == Nil) /* Elemen pertama yang dihapus */
        {
            First(*L) = Next(P);
        }
        else
